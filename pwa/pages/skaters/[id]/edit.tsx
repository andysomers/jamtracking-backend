...
const Page: NextComponentType<NextPageContext> = () => {
  ...
  const { data: { data: skater } = {} } = useQuery<
    FetchResponse<Skater> | undefined
  >(["skater", id], () => getSkater(id));
  ...
  return (
    <div>
      <div>
        <Head>
          <title>{skater && `Edit Skater ${skater["@id"]}`}</title>
        </Head>
      </div>
      <Form skater={skater} />
    </div>
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = await getItemPaths(response, "skaters", "/skaters/[id]/edit");
